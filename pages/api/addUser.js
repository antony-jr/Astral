...
const handler = (req, res) => {
  ...
  if (
	...
	if(username.toLowerCase() == 'administrator'){
    		res.statusCode = 200;
    		res.end(JSON.stringify({ error: false, reason: "cannot create reserved username" }));
    		return;
  	}
	...
	strict: true
	  ...
	  if (results.length == 0) {
		...
		if(typeof process.env.SENDGRID_API_KEY == 'undefined'){
			console.log("[warning] Please set SENDGRID_API_KEY to send emails.");
		}else{
			sgMail.setApiKey(process.env.SENDGRID_API_KEY);
			const HTML = siteConfig["account-notice"]["email-body"].replace
