...
const handler = (req, res) => {
  ...
  if(req.method != 'POST'){
	  res.statusCode = 200;
	  res.end(JSON.stringify({error : true, reason: "expected a post method request"}));
	  return;
  }
  ...
  if(typeof req.body.username == 'string' &&
   ...
   if (err) {
      ...
      con.query("SELECT * FROM Users WHERE UserID='" 
		      + req.body.username + "' and PwdHash='"+ 
		      hash +"';" , (error, results, fields) => {
      if (error) {
        con.release();
        res.statusCode = 200;
      ...
      res.statusCode = 200;
      if(results.length == 1){ // Implies successful login. 
	  ...
	  res.statusCode = 200;
	  res.end(JSON.stringify({error: true, reason: "invalid request body types, expected strings"}));
	  return;
  }
};
