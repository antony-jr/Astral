...
function UserPage(props) {
 ...
 React.useEffect(() => {
    axios
      ...
      .then(({ data }) => {
	      if(data.error){
		      setContent(<div className={classes.userPageRoot}>
			  Cannot connect to backend, Please contact the administrator.
		  </div>);
		      return;
	      }else if(data.length == 0){
		      setContent(<div className={classes.userPageRoot}>
			      You have no classes assigned, Please contact your administrator if you
 ...
 let render = (
	 <div>
		 <Typography variant="h4" >{props.legalName} / <b>Dashboard</b></Typography>
	 <Divider />
	 <br />
	 {content}
	 </div>
...
function Page(){
	...
	if(session && session.userLogged){
		if(session.username != 'administrator'){
			return <UserPage username={session.username} legalName={session.legalName} />
		}else{
