...
export default function GeneralSettings(props) {
  ...
  const handleSave = () => {
    ...
    axios.post("/api/setAcademicYear", params).then(({ data }) => {
      if (data.error) {
        setFailed(true);
  ...
  const handleReset = () => {
    axios.get("/api/getAcademicYear").then(({ data }) => {
      if (data.error) {
        setFailed(true);
        enqueueSnackbar("Cannot retrive data from backend.", {
          variant: "error"
        });
        console.log(data);
      } else {
        setSettings({
          ...settings,
          academicFromYear: data.fromAcademicYear.toString(),
          academicToYear: "" + data.toAcademicYear.toString()
        });
      }
    });
  };
