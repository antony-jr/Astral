...
export default function UserSettings(props) {
  ...
  const removeUsers = data => {
    ...
    for (var iter = 0; iter < data.length; ++iter) {
      ...
      axios.post("/api/removeUser", sendData).then(({ data }) => {
        if (data.error) {
  ...
  const submitAddUser = () => {
    if (username.length == 0 || email.length == 0 || legalName.length == 0) {
      setGenError(true);
    ...
    axios.post("/api/addUser", sendData).then(({ data }) => {
      if (data.error) {
  ...
  const refresh = () => {
    ...
    axios.get("/api/getUsers").then(({ data }) => {
      if (data.error) {
        enqueueSnackbar("Could not retrive user information", {
          variant: "error"
        });
      } else {
        setData(data.users);
